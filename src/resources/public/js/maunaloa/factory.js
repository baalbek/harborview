...
MAUNALOA.factory =  {
    ...
    getCanvasIdFor : function(reposId) {
        if (reposId === 1) {
            return this.DAY_LINES_OVERLAY;
        }
        else {
            return this.WEEK_LINES_OVERLAY;
        }
    },
    initRepos : function(reposId) {
        var repos = reposId === 1 ? this.repos1 : this.repos2;
        var canvasId = reposId === 1 ? this.DAY_LINES_OVERLAY : this.WEEK_LINES_OVERLAY;
        if (repos === null) {
          console.log("Creating repos " + reposId);
          repos = MAUNALOA.repos.create(canvasId,this.hruler,this.vruler);
          if (reposId === 1) {
            this.repos1 = repos;
          }
          else {
